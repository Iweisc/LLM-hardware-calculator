...
import ThemeToggle from './components/ThemeToggle'; // Add import for ThemeToggle
...
function App() {
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-200">
      <header className="header-gradient dark:header-gradient-dark"> {/* Add dark variant for gradient */}
        <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-2 text-white"> {/* Ensure header text is visible */}
                LLM Hardware Calculator
              </h1>
              <p className="text-primary-100 dark:text-primary-300 text-sm md:text-base max-w-xl"> {/* Adjust paragraph color */}
                Estimate GPU and system memory requirements for running large language models locally
              </p>
            </div>

            <div className="flex items-center space-x-4"> {/* Wrap badge and toggle */}
              <div className="model-badge mt-4 md:mt-0 animate-float text-white"> {/* Ensure badge text is visible */}
              ...
              <ThemeToggle theme={theme} toggleTheme={toggleTheme} /> {/* Add Theme Toggle Button */}
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 grid-cols-1 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            <h2 className="text-2xl font-semibold mb-6 text-gray-800 dark:text-gray-200 flex items-center"> {/* Dark text for heading */}
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 mr-2 text-primary-600 dark:text-primary-400"> {/* Adjust icon color */}
            ...
            {results && (
              <div className="space-y-8">
                ...
                {/* Memory Breakdown - Add dark styles */}
                <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-card animated-bg dark:animated-bg-dark"> {/* Adjust background, shadow, animation */}
                  <h3 className="text-lg font-medium text-gray-800 dark:text-gray-200 mb-4 flex items-center"> {/* Adjust text color */}
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2 text-primary-600 dark:text-primary-400"> {/* Adjust icon color */}
                ...
                {/* Recommended GPU Setup - Add dark styles */}
                <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-card animated-bg dark:animated-bg-dark"> {/* Adjust background, shadow, animation */}
                  <h3 className="text-lg font-medium text-gray-800 dark:text-gray-200 mb-4 flex items-center"> {/* Adjust text color */}
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-2 text-primary-600 dark:text-primary-400"> {/* Adjust icon color */}
      ...
      <footer className="footer-gradient dark:footer-gradient-dark mt-8 py-6"> {/* Add dark variant */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-center text-sm text-gray-600 dark:text-gray-400 font-medium"> {/* Adjust text color */}
              LLM Hardware Calculator - Estimate resources for running large language models locally
            </p>
            <p className="text-center text-xs text-gray-500 dark:text-gray-500 mt-2 md:mt-0"> {/* Adjust text color */}
